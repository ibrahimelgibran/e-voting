	...
	function dataPemilih(){
		$.ajax({
			type: 'GET', // Gunakan metode GET secara eksplisit
			url: '<?= base_url("admin/data_pemilih"); ?>',
			async: false, // Disarankan untuk dihindari, karena async false bisa menyebabkan UI freezing
			dataType: 'json',
			success: function(data){
				var html = '';
				if (data && data.length) {
	...
	$('#formtambah').on('submit', function(e){
		...
		var username = $('[name="username"]').val(); // Perbaikan: username harus mengambil value dari input username
	...
	$('#showData').on('click', '.hapus', function(){
		...
		if (k) {
			$.ajax({
				url: '<?= base_url("admin/hapus_pemilih/"); ?>'+id,
				success: function(){
					Swal.fire('Success', 'Voter successfully removed', 'success');
					dataPemilih();
				},
				error: function(xhr, status, error) {
